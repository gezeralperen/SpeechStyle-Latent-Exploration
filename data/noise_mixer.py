...
def build_from_path(in_dir, out_dir,
    ...
    def save_results(wav, f0_clean, duration, basename, basedir, dirtype, noise_name, snr, save_aux):
        ...
        if save_aux:
            # Save mel plot
            save_mel_plot(mel, os.path.join(basedir, "{}_{}_SNRdb_{}_{}.png".format(basename, dirtype, snr, noise_name.split('\\')[-1])), f0, energy)
            audiowrite(wav, hp.sampling_rate, os.path.join(basedir, "{}_{}_SNRdb_{}_{}.wav".format(basename, dirtype, snr, noise_name.split('\\')[-1])), norm=False)
            ...
            for f in filenames:
                if f.endswith(".wav"):
                    ...
                    if basename in unaligned_basenames or (basename not in (cleanbasenames_train+cleanbasenames_val)):
                        continue
                    speaker_id = str('_'.join(f.split('_')[:-1]))
                    audio_path = os.path.join(dirpath, f)

                    # It's important to reuse pre-calculated data e.g., duration, f0, energy.
                    tg_path = os.path.join(tg_dir, '_'.join(basename.split("_")[:-1]), basename+".TextGrid")
