...
def mfa(in_dir, out_dir, mfa_path="montreal-forced-aligner"): 
    ...
    cmd = f"mfa align {in_dir} {mfa_pretrain_path} english {mfa_out_dir} -j 8"
    print(cmd)
    ret = os.system("conda activate mfa\n" + cmd)
    print(ret)
...
def vocoder_infer(mel, vocoder, path, write=True):
    name = hp.vocoder
    with torch.no_grad():
        if name == "MelGAN":
